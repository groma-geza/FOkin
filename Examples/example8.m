...
[time, data, txt1] = create_Erlang_data(n, noise_sigma, with_exp, show);
...
load(fname)
% lambda = [1.E-15, 1]; % for sigma = 1.E-7
% lambda = [1.E-12, 1]; % for sigma = 1.E-5
% lambda = [1.E-9, 1]; % for sigma = 1.E-3
...
result.show() % Fig 12 A or C, S7 Fig A or C
save(fname, 'fokin', 'result', '-append')

%% Execute 10-fold CV in the 2D space of lambda end omega
load(fname)

% lambda = [1.E-14, 1]; % sigma 1.E-7
lambda = [1.E-8, 1]; % sigma 1.E-3
omega = [1.E-8, 1];
maxiter = 400;
info.dummy = 'dummy'; % dummy input info
...
result_kcv.show() % Fig 12 B or D, S7 Fig B or D
